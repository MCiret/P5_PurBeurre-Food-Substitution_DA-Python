...
def sort_write_json_resp_by_category(off_api_json_responses: 'list[dict[dict]]'):
    ...
    assert(type(off_api_json_responses) is list
           and len(off_api_json_responses) != 0)

    get_out_files_names_list = []

    for categories_dict in cfg.GET_QUERY_LIST_CATEGORIES_DICT:
        out_file_name = "DataLoading/" \
                        "Written json data/response_get_query_category"
...
def write_valid_products(valid_products: 'list[dict]'):
    with open("DataLoading/Written json data/final_products.json", "a",

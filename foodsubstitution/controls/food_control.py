...
class FoodControl(MenuControl):
    ...
    def run_menu(self, full_control: 'FullControl', user_input: int = None) -> 'int or str':
        ...
        if user_input:
            ...
            # if it is not a user menu return or if user returned but choose the same category twice in a row
            if not full_control.selected_category or selected_category != full_control.selected_category:
                full_control.selected_category = selected_category
                full_control.selected_category.foods_category = selected_category.foods_category \
                                                              = Food.objects.get_all_by_category(selected_category.id)

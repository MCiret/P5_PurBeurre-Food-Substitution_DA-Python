...
class FullControl:
    ...
    ATTRIBUTE_STR_TO_MENU_CONTROL_CLASS_DICT = {
        "main_menu_control": {"ct_class": "MainMenuControl", "next": ["cat_menu_control",
                                                                      "read_bookmarks_menu_control"]},
        ...
        "sub_menu_control": {"ct_class": "SubstitutionControl",
                             "next": ["bookmarking_menu_control"], "prev": ["food_menu_control"]},
    ...
    def call_menu_control(self, menu_control_name: 'str', user_input: 'int or str' = None):
        ...
        if not getattr(self, menu_control_name):
            setattr(self, menu_control_name,
                    globals()[FullControl.ATTRIBUTE_STR_TO_MENU_CONTROL_CLASS_DICT[menu_control_name]["ct_class"]]())
        ...
        if checked_user_input in cfg.GENERAL_MENU_VALID_INPUT_DICT["quit"]["val"]:
            ...
            if menu_control_name == "main_menu_control":
                self.call_menu_control(FullControl.ATTRIBUTE_STR_TO_MENU_CONTROL_CLASS_DICT
                                       [menu_control_name]["next"][checked_user_input - 1], checked_user_input)
            else:
                self.call_menu_control(FullControl.ATTRIBUTE_STR_TO_MENU_CONTROL_CLASS_DICT
                                       [menu_control_name]["next"][0], checked_user_input)

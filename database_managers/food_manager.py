...
class FoodManager:
    ...
    def get_all_by_category(self, category_id: int) -> 'list[Food]':
        """
        ...
        for food in curs.fetchall():  # fetchall() returns [] if query result is empty ; fetchnone() returns None
    ...
    def get_all_by_ctc_and_nutriscore_better_than(self, substituted_food: 'Food') -> 'list[SubstituteFood]':
        """
    ...
    def get_all_by_compared_to_category(self, substituted_food_id: int, substituted_food_ctc: str) -> 'list[Food]':
        """
    ...
    def save_bookmark(self, substitution_food_id: int, substituted_food_id: int) -> bool:
    ...
    def get_all_bookmarks_name_id_nutriscore(self) -> 'list[SubstitutionFood]':
        ...
        curs = self.db_connection.cursor()
        bk_list = []
        curs.execute("SELECT fs.barcode, fs.name, fs.quantity, fs.nutriscore, "
                     "fd.barcode, fd.name, fd.quantity, fd.nutriscore "
                     "FROM food as fs "
        ...
        for bookmark in curs.fetchall():
            bk_obj = m.SubstitutionFood(name=bookmark[1], quantity=bookmark[2], nutriscore=bookmark[3],
                                        substituted_food=m.Food(name=bookmark[5], quantity=bookmark[6],
                                        nutriscore=bookmark[7]))
    ...
    def insert_one_food(self, food_dict: dict) -> int:
        ...
        food_insert = ("INSERT INTO food"
                       ...
                       "VALUES (%s, %s, %s, %s, %s, %s)")  # using f-string directly does not escape the '
